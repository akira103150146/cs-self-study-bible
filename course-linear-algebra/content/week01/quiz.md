...
### 迷思對照
- **B** 以為兩條線一定會相交在某一點 → 觀念 1 的圖:三種情況
- **C** 以為「兩條方程式」就有兩個解 → 觀念 1 白話說:不可能剛好兩個解
- **D** 把「平行」和「重合」搞混 → 觀念 1 的圖:平行 vs 重合
...
### 為什麼
A 的領頭數都是 1、一列比一列靠右、所在的行其他位置都是 0。B 的第 2 個 pivot 上方是 2 不是 0;C 不是梯形(第 2 列的領頭數在左邊);D 的第 1 個領頭數是 2。

### 迷思對照
- **B** 忘了 pivot **上方**也必須是 0 → 觀念 3:RREF 多出來的兩個條件
- **C** 沒檢查領頭數要一列比一列靠右 → 觀念 3:樓梯形狀
- **D** 忘了 RREF 的領頭數必須是 1 → 觀念 3:RREF 多出來的兩個條件
...
### 迷思對照
- **A** 把全 0 列 $[\,0 \;\; 0 \;\; 0 \mid 0\,]$ 當成「$0 = $ 非零數」 → 觀念 4 的流程圖第一個問題
- **B** 看到兩個 pivot 就以為唯一解,沒有去數自由變數 → 觀念 4:基本變數與自由變數
...
### 迷思對照
- **$h \neq 3$** 把條件寫反,以為最後一列不是 0 才有解 → 觀念 4:$0 = b$ 什麼時候矛盾
