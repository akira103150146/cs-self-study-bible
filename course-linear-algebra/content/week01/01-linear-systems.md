...
Solve each system and describe its graph. (a) $x_1 - 2x_2 = -1,\;\; -x_1 + 3x_2 = 3$  (b) $x_1 - 2x_2 = -1,\;\; -x_1 + 2x_2 = 3$  (c) $x_1 - 2x_2 = -1,\;\; -x_1 + 2x_2 = 1$
...
4. $0 = 2$ 永遠不成立,所以沒有任何 $(x_1, x_2)$ 能同時滿足兩式——**無解**(不相容)。把兩式都改寫成 $x_2 = \frac12 x_1 + \cdots$ 的形式會看到:斜率都是 $\frac12$,截距不同,是兩條平行線。
5. (c) 兩式相加:$0 = 0$,永遠成立。這表示第二式其實就是第一式乘以 $-1$,兩條方程式描述**同一條直線**。
6. 所以 (c) 的解就是這條線上的每一個點,**無限多解**。例如 $(-1, 0)$、$(1, 1)$、$(3, 2)$ 都是解。
